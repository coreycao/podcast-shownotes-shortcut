...
async function rateLimit(request, env, target) {
  if (!env.RSS_PROXY_RATE_LIMIT) return null;

  const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
  const targetHost = safeHostname(target) || 'invalid';
  const key = `${ip}:${targetHost}`;
  const { success } = await env.RSS_PROXY_RATE_LIMIT.limit({ key });
